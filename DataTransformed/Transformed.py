...
def datatransformed():
    print("Iniciando proceso de DataTransformed...")
    path = os.getcwd()
    path = path.replace("\\", "\\\\")
    path = path.replace("\\\\", "/") + "/"
    path_data = path + "Data/"
    ...
    print("Lectura archivo data_total")
    ...
    df = df.groupby(['FECHA', 'PROVINCIA']).agg(
        {'NUM_CASOS':'sum', 'ALTITUD': 'mean', 'TEMP_MED': 'mean', 
         ...
         'PRES_MIN': 'mean', 'SOL': 'mean'}).reset_index()
    
    print("calculo población mensual")
    ...
    print("calculo tasa de incidencia")
