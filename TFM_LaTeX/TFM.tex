...
\\\\
Los resultados obtenidos con los distintos modelos se contrastaron basado en nuestras métricas definidas ($R^2$, MAE y RMSE), y se obtuvieron valores bajos en estas métricas, este resultado es el esperado, ya que las condiciones climáticas no son el factor principal en la propagación del virus, las variables no logran explicar nuestra variable a predecir. Sin embargo, el modelo de K-means evidencia una influencia de los factores climáticos sobre la tasa de incidencia, mediante una segmentación de los casos en 3 clusters, en donde los clúster con ciertas características climáticas presentara una tasa de incidencia diferente. 
\\\\
\\\\
...
\item Identificar la posible influencia de las condiciones climaticas en la propagación del virus de COVID-19 en España.
\end{itemize}
\medskip
...
\item Automatizar la metodología implementada en el desarrollo del proyecto.
...
\\
El modelo se entrenaría con datos históricos sobre la propagación del virus y las condiciones climáticas para predecir la propagación futura del virus en diferentes condiciones climáticas o más específicamente dependiendo del modelo utilizado una propuesta sería de la siguiente forma:
\\
\begin{itemize}
...
\begin{figure}[H]
	...
	\includegraphics[width=0.8\textwidth]{heatmap_clima}
		\textbf{\caption{{\small Headmap variables dataset clima. \label{tb:heatmap_clima}}}}
	{\scriptsize Fuente: [Elaboración propia]}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.7\textwidth]{tendencia_tasa_incidencia}
...
\begin{table}[H]
  \centering
   \textbf{\caption{{\small Tipos de datos dataset clima. \label{tb:tipo_datos_clima}}}}       
	\renewcommand{\arraystretch}{1.2}  
  %{\setlength{\arrayrulewidth}{0.3mm}
    \begin{tabular}{|c|c|}
    \hline
...
\begin{table}[H]
  ...
  %{\setlength{\arrayrulewidth}{0.3mm}
    ...
    \hline
    Seri Temporal Forecasterautoreg & $-6,46$ & $37937,62$ & $42756,51$ \\
	\hline
    \end{tabular}
    \label{tab:addlabel}\\
  {\scriptsize Fuente: Elaboración Propia}
